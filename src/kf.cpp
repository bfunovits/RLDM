// kf.cpp
// Kalman filter implementations for RLDM package
// Based on "parametrization/src/kf.cpp"
 ...
 *
 * Computes filtered states, predictions, and log-likelihood for a linear
 * Gaussian state space model:
 *   a_{t+1} = A a_t + w_t, w_t ~ N(0, Q)
 *   y_t = C a_t + v_t, v_t ~ N(0, R)
 *   Cov(w_t, v_t) = S
 *
 ...
 * @param y_t Observations (m x N), column-major organization
 * @param P1 Initial state covariance (s x s)
 * @param a1 Initial state estimate (s x 1)
 * @return Rcpp::List containing:
 *   - e: scaled residuals (N x m)
 *   - a: filtered states (N+1 x s)
 *   - ll: log-likelihood
 *   - P1: final state covariance
 *
 * @note Assumes column-major data organization for compatibility with R
 * @note y_t is (m x N) matrix (each column is one observation)
 */
// [[Rcpp::export]]
Rcpp::List kf_cpp(const arma::mat& A, const arma::mat& C,
                  const arma::mat& Q, const arma::mat& R,
                  const arma::mat& S, const arma::mat& y_t,
                  const arma::mat& P1, const arma::colvec& a1) {
  ...
  for (unsigned long int t = 0; t < N; t++) {
    ...
    Pt = A * Pt * tA + Q - K * tK;
    // Make sure that Pt is symmetric / is this necessary?
    Pt = (Pt + Pt.t()) / 2;
...
/**
 * @brief Alternative Kalman filter implementation using QR decomposition
 *
 * Implements Kalman filter using QR decomposition for improved numerical
 * stability. Uses Cholesky factor representation of state covariance.
 *
 * @param A State transition matrix (s x s)
 * @param C Observation matrix (m x s)
 * @param H_t Matrix parameter for QR-based implementation
 * @param y_t Observations (m x N), column-major organization
 * @param P1_R Right Cholesky factor of initial state covariance (s x s)
 * @param a1 Initial state estimate (s x 1)
 * @return Rcpp::List containing:
 *   - e: scaled residuals (N x m)
 *   - a: filtered states (N+1 x s)
 *   - ll: log-likelihood
 *   - P1: final state covariance as P1_R' * P1_R
 *
 * @note y_t is (m x N) matrix (each column is one observation)
 * @note Uses QR decomposition for numerical stability
 */
// [[Rcpp::export]]
Rcpp::List kf2_cpp(const arma::mat& A, const arma::mat& C,
                   const arma::mat& H_t, const arma::mat& y_t,
                   arma::mat& P1_R, const arma::colvec& a1) {
  ...
  for (unsigned long int t = 0; t < N; t++) {
    ...
    e.col(t) = solve(trimatl(sigma_L), e.col(t) - C * a.col(t));
    // State estimate a[t+1|t] = A a[t|t-1] + K ( y[t] - C a[t|t-1] )
    // = A a[t|t-1] + L e[t]
...
/**
 * @brief Compute log-likelihood using Kalman filter
 *
 * Computes the log-likelihood for a state space model using Kalman filter.
 * Includes early convergence detection when Kalman gain and covariance
 * stabilize.
 *
 * @param A State transition matrix (s x s)
 * @param C Observation matrix (m x s)
 ...
 *
 * @note Early convergence detection when max(abs(Pt - Pt0)) < tol and
 *       max(abs(K - K0)) < tol
 * @note For s=0 case, computes simple Gaussian log-likelihood
 */
// [[Rcpp::export]]
double ll_kf_cpp(const arma::mat& A, const arma::mat& C,
  ...
  while (!converged) {
    ...
    // Add log det Sigma[t|t-1] and sum of squared, scaled residuals to
    // log likelihood
    ...
    converged = ((t == N) or (((abs(Pt - Pt0)).max() < tol) and
                              ((abs(K - K0)).max() < tol)));
  }
...
/**
 * @brief Alternative log-likelihood computation using QR decomposition
 *
 * Computes log-likelihood using QR-based Kalman filter implementation
 ...
 *
 * @note Uses QR decomposition for numerical stability
 * @note Early convergence detection when max(abs(P1_R - P1_R0)) < tol
 */
// [[Rcpp::export]]
...
/**
 * @brief Compute log likelihood for statespace system described by template
 *
 * Internal helper function used by the function factory ll_FUN. Computes
 * log-likelihood for statespace systems parameterized by deep parameters
 * through linear templates.
 *
 * @param theta (K) dimensional vector of "deep" parameters
 * @param y (m,N) matrix with observed outputs: (y_1,y_2,...,y_N)
 * @param SYS (m+s,m+s) matrix, overwritten with system matrix [A,B | C,D]
 * @param H_SYS (m+s)^2, K) matrix
 * @param h_SYS ((m+s)^2)-dimensional vector. Note vec(SYS) = H_SYS*theta + h_SYS
 * @param sigma_L (m,m) matrix, overwritten with left square root of noise
 *        covariance matrix
 * @param H_sigma_L (m^2, K) matrix
 * @param h_sigma_L (m^2)-dimensional vector. Note vec(sigma_L) =
 *        H_sigma_L*theta + h_sigma_L
 * @param VAR (m+s,m+s) matrix, overwritten with covariance matrix
 *        [Q,S | S',R] = [B | C] sigma_L sigma_L' [B', C']
 * @param P1 (s,s) matrix, overwritten with initial state covariance matrix
 *        (computed via Lyapunov equation)
 * @param tol (double) tolerance used by ll_kf_cpp
 * @param err (double) return err if computation of P1 fails
 * @return double Log-likelihood value
 *
 * @note Uses rationalmatrices::lyapunov_impl for Lyapunov equation solution
 * @note Returns err if system is not stable (Lyapunov equation fails)
 */
// [[Rcpp::export]]
double ll_kf_theta_cpp(const arma::vec& theta, const arma::mat& y,
                       arma::mat& SYS, const arma::mat& H_SYS,
                       const arma::vec& h_SYS, arma::mat& sigma_L,
                       const arma::mat& H_sigma_L, const arma::vec& h_sigma_L,
                       arma::mat& VAR, arma::mat& P1, double tol,
                       double err) {
  ...
  // SYS_vec is the vectorised version of the matrix SYS (shares memory with SYS)
  arma::vec SYS_vec = arma::vec(SYS.memptr(), (m + s) * (m + s), false, true);
  // sigma_L_vec is the vectorised version of matrix sigma_L (shares memory)
  arma::vec sigma_L_vec =
      arma::vec(sigma_L.memptr(), (m) * (m), false, true);
  ...
  if (s > 0) {
    // Using header-only implementation from rationalmatrices for cross-package
    // linking
    bool ok = rationalmatrices::lyapunov_impl(
        SYS.submat(0, 0, s - 1, s - 1), VAR.submat(0, 0, s - 1, s - 1), P1,
        lambda_r, lambda_i, true);
    if (!ok) {
  ...
  // Rcout << SYS.submat(0,0,s-1,s-1) << VAR.submat(0,0,s-1,s-1) << P1 <<
  // std::endl;

  double ll;
  if (s > 0) {
    //             A,                       C
    ll = ll_kf_cpp(SYS.submat(0, 0, s - 1, s - 1),
                   SYS.submat(s, 0, s + m - 1, s - 1),
                   //             Q                        R
                   VAR.submat(0, 0, s - 1, s - 1),
                   VAR.submat(s, s, s + m - 1, s + m - 1),
                   //             S
                   VAR.submat(0, s, s - 1, s + m - 1), y, P1, a1, tol);
  } else {
    //             A,              C
    ll = ll_kf_cpp(arma::mat(0, 0), arma::mat(m, 0),
                   //             Q               R    S
                   arma::mat(0, 0), VAR, arma::mat(0, m), y, P1, a1, tol);
  }
