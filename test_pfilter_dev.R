...
cat("Testing particle filter implementation (dev version)...\n")
...
# 2. Particle filter (SIR) with more particles
...
cat("\n3. Particle filter (APF)...\n")
pf_apf_result <- pfilter(model, y, N_particles = 5000, method = "apf")
cat("APF log-likelihood:", pf_apf_result$log_likelihood, "\n")

# 4. Likelihood approximation
cat("\n4. Likelihood approximation (averaged)...\n")
ll_pf_result <- ll_pfilter(model, y, N_particles = 2000, N_runs = 5)
...
for (resamp in c("systematic", "multinomial", "stratified")) {
  ...
  res <- try(pfilter(model, y, N_particles = 1000, method = "sir", resampling = resamp))
