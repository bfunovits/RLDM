...
must_haves:
  ...
  artifacts:
    - path: "inst/_pkgdown.yml"
      ...
      contains: "reference:" (but not "outputs_ARMA_cpp", "outputs_STSP_cpp", "residuals_ARMA_cpp", "residuals_STSP_cpp", "cll_theta_ARMA_cpp", "cll_theta_STSP_cpp", "fbsolve_STSP_cpp", "solve_rmfd_cpp")
...
# Research identified 8 internal C++ functions referenced in pkgdown config without Rd documentation:
# - outputs_ARMA_cpp
# - outputs_STSP_cpp
# - residuals_ARMA_cpp
# - residuals_STSP_cpp
...
2. Identify the section "RcppArmadillo Implementations - Solving" (lines 100-109 in current version)
3. Remove the 8 internal C++ function references that lack Rd documentation:
   - outputs_ARMA_cpp
   - outputs_STSP_cpp
   - residuals_ARMA_cpp
   - residuals_STSP_cpp
   ...
   - fbsolve_STSP_cpp
   - solve_rmfd_cpp
4. Options for handling the empty section:
   ...
   **Choose Option A:** Remove the entire section since these are internal C++ functions without public documentation. The wrapper functions (kf(), ll_kf(), etc.) are already documented elsewhere.

5. Save the updated configuration
6. Verify the YAML is still valid (no syntax errors)
  </action>
  <verify>Updated inst/_pkgdown.yml no longer contains references to outputs_ARMA_cpp, outputs_STSP_cpp, residuals_ARMA_cpp, residuals_STSP_cpp, cll_theta_ARMA_cpp, cll_theta_STSP_cpp, fbsolve_STSP_cpp, solve_rmfd_cpp</verify>
  <done>pkgdown configuration fixed with invalid references removed, ready for local build test</done>
</task>
...
- pkgdown configuration fixed (invalid references removed)
