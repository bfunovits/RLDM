...
<task type="auto">
  <name>Task 3: Investigate vignette build failures</name>
  <files>.planning/phases/03-build-verification/vignette-issues.md</files>
  <action>
1. Analyze vignette build errors from check-initial.log:
   - Extract specific error messages related to vignettes
   - Identify if errors are due to missing system dependencies (pandoc-citeproc) or code issues (coercion errors)
   - Categorize each issue with root cause

2. Create vignette-issues.md documenting findings:
   - List each vignette that fails to build
   - Document error messages and root causes
   - Note whether issue is system dependency (external) or code issue (internal)
   - For system dependencies: document installation instructions
   - For code issues: note specific files/lines needing fixes

3. Determine action plan for Plan 03-02:
   - If system dependencies missing: document as external issue, not blocking BUILD-01
   - If code issues fixable: add fix tasks to Plan 03-02
   - If complex issues: document for future phases

Why: Vignette build failures could prevent BUILD-01 (0 errors). Need to determine if they're fixable in this phase or should be documented as external/system issues.
  </action>
  <verify>
- File `.planning/phases/03-build-verification/vignette-issues.md` exists with analysis
- Each vignette failure categorized with root cause
- Clear distinction between system dependency vs code issues
- Action plan documented for Plan 03-02
  </verify>
  <done>Vignette failures analyzed and documented with root causes and recommended actions.</done>
</task>
...
<success_criteria>
- [ ] `check-initial.log` file exists with ≥20 lines
...
- [ ] `vignette-issues.md` exists with analysis of vignette failures
</success_criteria>
...
- Vignette failure analysis and recommendations
