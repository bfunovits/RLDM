...
must_haves:
  truths:
    ...
    - "LaTeX errors in PDF manual are fixed (forgetting_factors issue)"
  artifacts:
    ...
    - path: "man/arx_rls_core.Rd"
      provides: "Fixed LaTeX syntax for forgetting_factors"
      contains: ["\\eqn{(#forgetting_factors \\times 1)}"]
  key_links:
...
# 4. PDF manual LaTeX error: forgetting_factors syntax issue
</context>
...
3. **Fix solve_rmfd_cpp cross-references:**
   - Search for references to `solve_rmfd_cpp` in R files
   - Ensure they use proper `\link{}` syntax pointing to wrapper functions

4. **Fix forgetting_factors LaTeX error:**
   - In R/05_estimation_rls.R line 27: `\eqn{ (#forgetting_factors \times 1) }`
   - Change to: `\eqn{(#forgetting_factors \times 1)}` (remove space after opening brace)
   - LaTeX error: "You can't use `macro parameter character #' in math mode"
   - Solution: Ensure `#` is properly escaped or formatted
...
2. Check `check-documentation.log` for "Missing link(s)" and "Documented arguments not in \usage" warnings - should be 0
3. Check PDF manual LaTeX errors - should be 0 (specifically forgetting_factors issue resolved)
...
- [ ] 0 PDF manual LaTeX errors (forgetting_factors syntax fixed)
