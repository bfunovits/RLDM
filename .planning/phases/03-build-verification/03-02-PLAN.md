...
wave: 2
depends_on: ["03-01"]
files_modified: ["DESCRIPTION", "NAMESPACE", "R/*.R", "man/*.Rd"]
autonomous: true
user_setup: []
...
<task type="auto">
  ...
  <files>DESCRIPTION, NAMESPACE, R/*.R, man/*.Rd, .planning/phases/03-build-verification/issues-fixed.md</files>
  <action>
...
Why: Automatic fixes for common issues reduce warning count. Namespace fixes are critical for package functionality. Documentation fixes improve package quality.
  </action>
  <verify>
...
- No obvious namespace issues remain (scan for common patterns)
  </verify>
...
- `check-final.log` shows WARNING: ≤1 (excluding acceptable warnings)
...
1. Check `check-final.log` for success:
   - "ERROR" count: 0 ✓
   - "WARNING" count: ≤1 ✓ (excluding non-ASCII, time verification)
...
- [ ] `check-final.log` shows ≤1 warning (excluding acceptable warnings)
